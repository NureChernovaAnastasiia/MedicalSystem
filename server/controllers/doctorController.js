...
class DoctorController {
  ...
  async create(req, res, next) {
    try {
      ...
      if (req.user.role !== "Admin") {
        return next(
          ApiError.forbidden(
            "Тільки адміністратор може створювати лікарів вручну"
          )
        );
      }
      ...
      if (!user || user.role !== "Doctor") {
        return next(
          ApiError.badRequest(
            "Користувач з таким email не є лікарем або не існує"
          )
        );
      }
  ...
  async updatePhoto(req, res, next) {
    try {
      ...
      if (!photo) {
        return next(ApiError.badRequest("Необхідно передати фото"));
      }
      ...
      // Доступ тільки для Admin або самого лікаря
      if (req.user.role !== "Admin" && req.user.id !== doctor.user_id) {
        return next(ApiError.forbidden("Немає доступу до оновлення фото"));
      }

      doctor.photo_url = photo;
      await doctor.save();

      return res.json({ message: "Фото успішно оновлено", doctor });
    } catch (e) {
      console.error("updatePhoto error:", e);
      return next(ApiError.internal("Помилка оновлення фото лікаря"));
    }
  }
}
