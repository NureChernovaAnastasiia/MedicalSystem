const { Prescription, Patient } = require('../models/models');
...
class PrescriptionController {
  async getAll(req, res, next) {
    try {
      if (req.user.role !== 'Doctor' && req.user.role !== 'Admin') {
        return next(ApiError.forbidden("Тільки лікар або адміністратор можуть переглядати всі рецепти"));
      }
  ...
  async getByPatient(req, res, next) {
    try {
      ...
      const items = await Prescription.findAll({ where: { patient_id: patientId } });
      return res.json(items);
    } catch (e) {
  ...
  async create(req, res, next) {
    try {
      if (req.user.role !== 'Doctor' && req.user.role !== 'Admin') {
        return next(ApiError.forbidden("Тільки лікар або адміністратор можуть створювати рецепт"));
      }
  ...
  async update(req, res, next) {
    try {
      if (req.user.role !== 'Doctor' && req.user.role !== 'Admin') {
        return next(ApiError.forbidden("Тільки лікар або адміністратор можуть оновлювати рецепт"));
      }
      const item = await Prescription.findByPk(req.params.id);
      if (!item) return next(ApiError.notFound('Рецепт не знайдено'));
  ...
  async delete(req, res, next) {
    try {
      if (req.user.role !== 'Doctor' && req.user.role !== 'Admin') {
        return next(ApiError.forbidden("Тільки лікар або адміністратор можуть видаляти рецепт"));
      }
      const item = await Prescription.findByPk(req.params.id);
      if (!item) return next(ApiError.notFound('Рецепт не знайдено'));
      await item.destroy();
